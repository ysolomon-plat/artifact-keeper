...
- The RPM Phase-1 connect-time private-IP check runs in the Upstream resolver context and, as a fail-safe, also blocks private-IP targets for **SSO-metadata** and **webhook** flows that carry their own private-IP allow-toggles. It **fails safe** (over-blocks, never leaks). Reconciled by **#2380** (see *Fixed* below): those flows now honor their per-surface toggles at connect time.

### Fixed

- **Connect-time SSRF check honors the per-surface private-IP toggles for webhook delivery and SSO/OIDC fetches** (#2380): the resolver-level private-IP gate introduced in #2354 ran in the Upstream trust class for every outbound client, so `WEBHOOK_ALLOW_PRIVATE_IPS` / `SSO_ALLOW_PRIVATE_IPS` (and the shared `AK_SSRF_ALLOW_PRIVATE_CIDRS` allowlist) were ignored at connect time and legitimate private-IP webhook / SSO-metadata targets were over-blocked. Webhook delivery and SSO/OIDC identity-provider fetches now use dedicated HTTP clients whose connect-time DNS check and per-hop redirect policy run in their own trust class and honor the corresponding toggle. Defaults are unchanged (fail-closed with no toggle set); cloud-metadata, loopback and link-local addresses stay hard-blocked regardless of any toggle; the RPM/upstream proxy path is untouched.
